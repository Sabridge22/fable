...
3. **Начинает диалог** - отправляет сообщение через эндпоинт `POST /chat`.
...
5. **Пользователь может посмотреть историю** - все диалоги сохраняются и доступны через эндпоинт `GET /chat`.
